...
* 시스템 환경에서 디스크 상태를 점검(`lsblk -S` 등)합니다.
  ![디스크 상태 점검](ch.6%20디스크%20관리와%20공간%20할당/raid1/6.solve_add_disk.png)
* 장애가 난 자리에 정상 디스크(`/dev/sdb1`)를 다시 추가(`--add`)하여 패리티 동기화 및 복구 작업을 수행합니다.
  ![디스크 추가 및 복구](ch.6%20디스크%20관리와%20공간%20할당/raid1/7.add_disk_inRaid.png)
...
* `/raid1` 경로에 들어가 장애 발생 이전에 생성했던 `testFile`이 손실 없이 그대로 남아있는지 최종 확인합니다.
  ![데이터 최종 검증](ch.6%20디스크%20관리와%20공간%20할당/raid1/8.check_data.png)